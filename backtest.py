...
def run_backtest():
    ...
    print("Processando Inferências de Machine Learning...")
    ...
    print("Iniciando Simulação Event-Driven...")
    ...
    for timestamp, row in df.iterrows():
        ...
        if position['active']:
            ...
            # Trailing Stop: Puxa o stop para o Break-even após 2.0 * ATR de lucro bruto
            if position['highest_price'] >= position['avg_price'] + (2.0 * position['atr_value']):
                break_even_lock = position['avg_price'] * (1 + TAKER_FEE * 2) # cobre taxas
            ...
            # Defesa Ativa do Machine Learning (Saídas Fracionadas)
            if not sold:
        ...
        if not position['active']:
            ...
            decision = "NEUTRO"
            if buy_score >= 60: decision = "COMPRA_FORTE"
            elif buy_score >= 35: decision = "COMPRA_MODERADA"
            
            if decision in ["COMPRA_FORTE", "COMPRA_MODERADA"] and ml_prob_success >= 0.10 and atr_pct >= 0.004:
                ...
                if invested_usdt > 11.0:
                    ...
                    # Stop Loss Inicial: 0.5 * ATR e Take Profit 3.0 * ATR
                    atr_val = atr if atr > 0 else (current_price * 0.002)
                    position['atr_value'] = atr_val
                    position['sl_price'] = current_price - (0.5 * atr_val)
    ...
    print(f"Saldo Final:   ${final_equity:.2f}")
    print(f"Retorno Total: {total_return:.2f}%")
    print(f"Total de Trades (Entradas): {total_trades}")
    print(f"Max Drawdown:  {max_dd:.2f}%")
    print("="*40)

    print("Gerando Relatório Visual (Plotly)...")
    
    # Reduzir a carga de memória do Plotly para os últimos 10.000 minutos (Aproximadamente 7 dias)
    plot_df = df.tail(10000)
    
    fig = go.Figure()
    
    fig.add_trace(go.Scatter(x=plot_df.index, y=plot_df['close'], mode='lines', name='Price', line=dict(color='gray', width=1)))
    
    if buy_markers:
        valid_buys = [(x, y) for x, y in buy_markers if x >= plot_df.index[0]]
        if valid_buys:
            bx, by = zip(*valid_buys)
            fig.add_trace(go.Scatter(x=bx, y=by, mode='markers', name='Buy', marker=dict(color='lime', size=12, symbol='triangle-up')))
            
    if sell_markers:
        valid_sells = [(x, y) for x, y in sell_markers if x >= plot_df.index[0]]
        if valid_sells:
            sx, sy = zip(*valid_sells)
            fig.add_trace(go.Scatter(x=sx, y=sy, mode='markers', name='Sell/TP', marker=dict(color='red', size=12, symbol='triangle-down')))

    fig.update_layout(
        title="Backtest Report - Price & Trades (Últimos 7 dias)", 
        xaxis_title="Time", 
        yaxis_title="Price", 
