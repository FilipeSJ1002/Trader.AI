...
def simulate(records, params):
    ...
    for row in records:
        ...
        if active:
            ...
            if highest_price >= avg_price + (trail_dist * atr_value):
    ...
    return ((final_equity - initial_balance) / initial_balance) * 100, params
...
def run():
    ...
    df['atr'] = df[atr_col]
    df['ema_200'] = df[[c for c in cols if c.startswith('EMA_200')][0]]
    ...
    print("Running PUSH TARGET MULTIPROCESSING grid...")
    grid = {
        'sl_mult': [0.5, 1.0, 1.5],
        'tp_mult': [1.5, 2.0, 2.5, 3.0],
        'ml_thresh': [0.1, 0.4],
        'atr_thresh': [0.003, 0.004, 0.005, 0.006],
        'buy_thresh': [35, 45, 60],
        'trail_dist': [1.0, 1.5, 2.0]
    }
    ...
    saida = Parallel(n_jobs=-1, verbose=1)(delayed(simulate)(records, dict(zip(keys, vals))) for vals in combinations)
    results: list = list(saida) if saida is not None else []
    
    best_return = -100
    best_params = None
    
    for ret, params in results:
        if ret > best_return:
            best_return = ret
            best_params = params
